...
class PHPCalendar
{
	...
	private $holidayDay=array(30,31);
	private $holidayMonth=array(12);
	...
	function getCalendarHTML()
	{
		...
		$calendarHTML .= '</div>';
		$calendarHTML .= '<div class="select1">';
		$calendarHTML .= '<select id="month">
        ...
        <option value="12">Dec</option>
		   ...
		   ';
		$calendarHTML .= '<div class="button">GET</div>';
		$calendarHTML .= '</div>';
		return $calendarHTML;
	}
	...
	{	
		...
		for ($i = 0; $i < $weekLength; $i++) {
			for ($j = 1; $j <= 7; $j++) {
				...
				if($this->currentMonth==12&&($this->currentDay==31||$this->currentDay==30||$this->currentDay==29)){
					$weekDays .= '<li' .' class="holiday">'. $cellValue . '</li>';
				}else{
