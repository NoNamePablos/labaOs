...
--SELECT * FROM dbo.Bank_Types
--WHERE id_bank BETWEEN '26' AND '28'
---- 4. Запрос с единичной агрегатной функцией (count, sum, avg, max, min)
SELECT min(age) as [Возраст] FROM dbo.Client
--иллюстрация возможности,если в первом запросе нельзя использовать составные,то во 2 можно
...
---- Вывести количество побед у каждого игрока

SELECT age FROM dbo.Client GROUP BY age
----  у кого больше 1 победы
SELECT id_client, SUM(count_visit) as sum FROM dbo.[Date of Visit Client] GROUP BY id_client having id_client=10007
...
---- 8. Составной запрос
---- Вывод игроков, у которых больше трёх побед за чёрных (ChessPlayer2)
